...
## 보안 리뷰 백로그 (P6 통합 리뷰, 2026-07-06 — 비차단 WARN)

멀티 매장(프랜차이즈) 확장 Phase 1~4 완료 후 진행한 `security-expert` 통합 보안 리뷰 결과:
**FAIL 0건, PASS(low 심각도 WARN 3건)**. 즉시 수정 불필요하나 추적을 위해 기록한다.

| # | 항목 | 위치 | 내용 | 상태 |
|---|---|---|---|---|
| 1 | i18n 로케일 프리픽스 암묵 의존 | `src/i18n/routing.ts` | `localePrefix: "always"`가 명시적으로 설정되어 있지 않고 암묵적으로 의존 중. 프록시의 `x-pathname` 로직이 이 설정값에 의존하므로, next-intl 기본값이 바뀌면 라우팅이 조용히 깨질 수 있다. | OPEN |
| 2 | Store 유니크 위반 미처리 | `src/app/api/admin/stores/route.ts` (POST/PATCH) | `Store.name`의 `@@unique([name])` 위반(Prisma `P2002`)을 명시적으로 캐치하지 않아, 매장명 중복 생성/수정 시 매핑되지 않은 500 에러가 발생할 수 있다. `DELETE` 핸들러의 `P2003`(STORE_IN_USE) 처리와 동일한 패턴으로 `P2002` → 4xx 매핑 추가 권장(예: `STORE_NAME_TAKEN`, 409). POST/PATCH에서 `P2002` 캐치 후 409 `STORE_NAME_DUPLICATE` 응답으로 해소, `e2e/tests/admin/stores.spec.ts` TC-74 검증 완료(security-expert 재검토: 정보 노출 없음, 승인). | FIXED (2026-07-07) |
| 3 | requireAdmin 401/403 혼용 | `src/lib/middleware/admin-auth.ts` — `requireAdmin()` | 미인증(세션 없음)과 권한부족(레벨 미달)을 모두 401로 응답한다. `resolveStoreScope()`(`src/lib/middleware/store-scope.ts`)는 이미 미인증=401 / 권한부족=403으로 구분하고 있어 두 헬퍼 간 시맨틱이 불일치한다. 권한부족은 403이 더 정확한 시맨틱이며, 향후 `requireAdmin`도 동일 패턴으로 정렬 검토. | OPEN |

> 다음 보안 리뷰 사이클에서 상태를 `OPEN` → `FIXED`/`WONTFIX`로 갱신할 것 (`security-expert` 소관).
