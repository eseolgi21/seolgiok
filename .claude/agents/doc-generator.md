...
### 6단계 — DB 스키마 문서
Agent 도구로 **db-expert** 에이전트를 호출:
```
이 프로젝트의 Prisma 스키마(prisma/schema/ 9개 파일)를 분석해
DB 모델 구조, 관계, 인덱스, enum 정의를 docs/specs/database.md 에 문서화해줘.
파일이 이미 있으면 Edit으로 업데이트, 없으면 Write로 생성.
문서 상단에 반드시 <!-- Last updated: YYYY-MM-DD --> 주석 포함.
```

### 7단계 — 다국어(i18n) 문서
Agent 도구로 **i18n-expert** 에이전트를 호출:
```
이 프로젝트의 src/i18n/ 디렉터리를 분석해
5개 언어 구조, 네임스페이스 목록, 키 명명 규칙, 번역 워크플로우를
docs/specs/i18n.md 에 문서화해줘.
파일이 이미 있으면 Edit으로 업데이트, 없으면 Write로 생성.
문서 상단에 반드시 <!-- Last updated: YYYY-MM-DD --> 주석 포함.
```

### 8단계 — 보안 가이드
Agent 도구로 **security-expert** 에이전트를 호출:
```
이 프로젝트의 src/lib/auth/, src/lib/crypto.ts 를 분석해
인증 흐름, 암호화 패턴, OTP 설정, 보안 규칙을
docs/specs/security.md 에 문서화해줘.
파일이 이미 있으면 Edit으로 업데이트, 없으면 Write로 생성.
문서 상단에 반드시 <!-- Last updated: YYYY-MM-DD --> 주석 포함.
```
...
📄 docs/ 문서 생성 완료
  ...
  ✅ docs/specs/database.md     — db-expert
  ✅ docs/specs/i18n.md         — i18n-expert
  ✅ docs/specs/security.md     — security-expert
