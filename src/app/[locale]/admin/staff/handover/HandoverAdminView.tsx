...
import {
  ...
  InputLabel, Chip, Tabs, Tab, Alert,
} from "@mui/material";
...
function SlotManager({ category, label, storeId }: { category: "HALL" | "KITCHEN"; label: string; storeId: string }) {
  const [slots, setSlots] = useState<Slot[]>([]);
  const [newLabel, setNewLabel] = useState("");

  const fetchSlots = () =>
    fetch(`/api/admin/staff/handover/slots?category=${category}${storeId ? `&storeId=${storeId}` : ""}`)
      .then((r) => r.json())
      .then((d) => { if (d.ok) setSlots(d.slots); });

  useEffect(() => { fetchSlots(); }, [storeId]);
  ...
  const handleAdd = async () => {
    ...
    const res = await fetch("/api/admin/staff/handover/slots", {
      ...
      body: JSON.stringify({ label: newLabel.trim(), category, ...(storeId ? { storeId } : {}) }),
    });
...
export default function HandoverAdminView({ isAdmin }: { isAdmin: boolean }) {
  ...
  const fetchItems = () =>
    fetch(`/api/admin/staff/handover/items${storeId ? `?storeId=${storeId}` : ""}`)
      .then((r) => r.json())
      .then((d) => {
        if (!d.ok && d.code === "STORE_NOT_ASSIGNED") { setStoreNotAssigned(true); return; }
  ...
  const handleAdd = async () => {
    ...
    const res = await fetch("/api/admin/staff/handover/items", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ label: newLabel.trim(), category: newCategory, ...(storeId ? { storeId } : {}) }),
    });
    ...
    <Box>
      ...
      {tabIdx === 1 && (
        <>
          ...
          {slotCatIdx === 0 && <SlotManager category="HALL" label="홀" storeId={storeId} />}
          {slotCatIdx === 1 && <SlotManager category="KITCHEN" label="주방" storeId={storeId} />}
        </>
      )}
      </>
      )}
