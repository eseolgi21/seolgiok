...
export async function DELETE(req: NextRequest) {
    ...
    try {
        ...
        // NEW: Delete All EXCEPT Keywords
        if (body.action === "DELETE_EXCEPT") {
            ...
            const res = await prisma.purchaseItem.deleteMany({
                where: {
                    confirmed: false,
                    NOT: {
                        OR: expandedKeywords.flatMap(k => [
...
export async function POST(req: NextRequest) {
    ...
    try {
        const body = await req.json();
        const { date, category, itemName, amount, note } = body;
        ...
        const newItem = await prisma.purchaseItem.create({
            data: {
                ...
                note: note || "",
                confirmed: false // Manual add is pending by default so it shows in the list
            }
        });
...
export async function PATCH(req: NextRequest) {
    ...
    try {
        ...
        // If updating amount, ensure it's a number
